...
%\tablecaption{Priors for parameters in $\Theta$. \tablabel{bounds}}
%\tablewidth{0pt}
%\tablehead{\colhead{Parameter} & \colhead{Prior}}
%\startdata
%$z$ & Fixed by catalogue \\[2pt]
%$\ddl$ & Fixed by $z$ and cosmology\tablenotemark{a} \\[2pt]
...
%$\xN$ & Fixed at $1.0$ 
%\enddata
%\tablenotetext{a}{We use a benchmark $\Lambda$CDM cosmology with $H_0 = 71\text{km}\ \text{s}^{-1} \text{Mpc}^{-1}$ and $\Omega_m = 0.27$.}
%\tablecomments{Parameters which are not fixed are given a uniform prior within the bounds above.}
%\end{deluxetable}
...
%\begin{deluxetable}{cc}
%\tablecaption{Number of light curves passing the ``well-fit'' criteria for each parameter. \tablabel{passed} }
%\tablewidth{0pt}
%\tablehead{\colhead{Parameter} & \colhead{Number of Well Fit Light Curves}}
%\startdata
%$\thO$ & $53$  \\
%$\thobs / \thO$ & $63$ \\
%$p $& $128$ \\
...
%\tablecaption{Quantiles on distribution of fit values for $\thO$, $\thobs / \thO$, and $p$. \tablabel{quantiles} }
%\tablewidth{0pt}
%\tablehead{\colhead{Parameter} & \colhead{Median} & \colhead{68\% Quantile} & \colhead{95\% Quantile}}
%\startdata
