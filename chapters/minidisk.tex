...
\end{tabular}
\end{center}
\caption{\tablabel{models} Minidisk models.}
\end{table}

%\begin{deluxetable}{ccccccc}
%\tablecaption{Minidisk Models \label{tb:models}}
%%\tablewidth{0pt}
%\tablehead{\colhead{Name} &\colhead{$\dot{M}$\tablenotemark{a}} & \colhead{$r_\text{in}$} & \colhead{$q$} & \colhead{$N_r$} & \colhead{$T_\text{start}$\tablenotemark{b}} & \colhead{$T_\text{end}$\tablenotemark{b}}}
%\startdata
...
%\tablenotetext{b}{$T_\text{start}$ and $T_\text{end}$ are given in units of $T_\text{bin}$.}
%\end{deluxetable}
